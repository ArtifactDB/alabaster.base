# Test loadAllObjects on various bits and pieces.
# library(testthat); library(alabaster.base); source("test-loadAllObjects.R")
...
test_that("loadAllMetadata works as expected", {
    all.meta <- loadAllMetadata(tmp, ignore.children=FALSE)
    ...
    child.meta <- loadAllMetadata(tmp, ignore.children=TRUE)
    all.meta <- all.meta[!is_child]
    expect_identical(child.meta, all.meta)
    expect_true(length(child.meta) > 0)
})

test_that("loadAllObjects works as expected", {
    all.obj <- loadAllObjects(tmp, redirect.action="both")
    ...
    to.obj <- loadAllObjects(tmp, redirect.action="to")
    expect_identical(to.obj, all.obj[setdiff(names(all.obj), "whoop")])

    from.obj <- loadAllObjects(tmp, redirect.action="from")
