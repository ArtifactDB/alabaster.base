...
Advanced users can exploit this by loading components from subdirectories if that part of the object is required:
...
Applications should call `altSaveObjectFunction()` to instruct `altSaveObject()` to use this generic.
...
In general, applications should avoid modifying the files created by `saveObject()`, to avoid violating any **takane** format specifications 
